#!/usr/bin/env node
// Syncs the vendored packages/uk-sources from a checkout of
...
// uses @uklab for its own packages, so the copy is renamed on the way in.
...
const CONNECTORS_PACKAGE_DIR = 'packages/uk-sources';
const DEFAULT_FROM = resolve(REPO_ROOT, '..', 'uk-open-data-connectors');
const SOURCE_PACKAGE_NAME = '@nzlab/uk-sources';
const LOCAL_PACKAGE_NAME = '@uklab/uk-sources';
const SYNCED_FILES = [
  'README.md',
  'eslint.config.mjs',
  'package.json',
...
// Rewrites the @nzlab scope to @uklab in the copied package, so the vendored
...
function renameScopeInCopy(packageRoot) {
  ...
  for (const filePath of listFiles(packageRoot)) {
    ...
    const before = readFileSync(filePath, 'utf8');
    const after = before.replaceAll('@nzlab/', '@uklab/');
    if (after !== before) {
      writeFileSync(filePath, after);
